<!DOCTYPE html>
<html lang="ru">

<head>
    <meta charset="utf-8">
    <title>Доставка суши, роллов и пиццы Кострома - МиМи суши</title>
    <link rel="stylesheet" href="css/style.css">
    <link rel="stylesheet" href="css/style2.css">
    <link rel="stylesheet" href="css/font-awesome-4.7.0/css/font-awesome.min.css">
    <script type="text/javascript" src="jquery-3.6.3.min.js"></script>
    <meta charset="UTF-8">
    
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="Бесплатная доставка суши, роллов и пиццы на дом — вкусно, дешево, быстро, заказ онлайн или по телефону. Мы готовим только из свежих продуктов высокого качества.">
    <meta name="keywords" content="доставка пиццы, заказ пиццы, пицца круглосуточно, суши, роллы, доставка суши, заказ суши, суши круглосуточно, доставка роллов, заказ роллов, роллы круглосуточно">
    
</head>

<body>
<?php
include('nav.php')
?>

        <section class="fon-back">
        <div class="mytext-centr"><h2 class="menu-centr">МЕНЮ</h2></div>


    <div class="cards" >
    
                  
<!-- Карточка товара -->
     <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="rolls.php" class="card__image">
      <img
        src="./img/cart-rolls.jpg"
        alt="Роллы"
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    <!-- Цены на товар (с учетом скидки и без)-->
    
    <!-- Ссылка-название товара -->
    <a href="rolls.php" class="card__title">
     Роллы
    </a>
    <!-- Кнопка добавить в корзину -->
    <a href=rolls.php class="roll-add2"><button class="card__add">Выбрать</button></a>
  </div>
</div>
    
    <!-- Карточка товара -->
     <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="#" class="card__image">
      <img
        src="img/pizza-card2.jpg"
        alt="Роллы"
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    <!-- Цены на товар (с учетом скидки и без)-->
    
    <!-- Ссылка-название товара -->
    <a href="#" class="card__title">
     Пицца
    </a>
    <!-- Кнопка добавить в корзину -->
    <button class="card__add">Выбрать</button>
  </div>
</div>

   <!-- Карточка товара -->
     <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="sushi.php" class="card__image">
      <img
        src="img/sushi-card.jpg">
        alt="Роллы"
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    
    <!-- Ссылка-название товара -->
    <a href="sushi.php" class="card__title">
     Суши
    </a>
    <!-- Кнопка добавить в корзину -->
    <a href=sushi.php class="roll-add2"><button class="card__add">Выбрать</button></a>
  </div>
</div>
  
  
  <!-- Карточка товара -->
     <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="fastfood.php" class="card__image">
      <img
        src="img/fastfood-card.png"
        alt="Роллы"
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    <!-- Цены на товар (с учетом скидки и без)-->
    
    <!-- Ссылка-название товара -->
    <a href="fastfood.php" class="card__title">
     Фастфуд
    </a>
    <!-- Кнопка добавить в корзину -->
    <a href=fastfood.php class="roll-add2"><button class="card__add">Выбрать</button></a>
  </div>
</div>
   
   <!-- Карточка товара -->
     <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="sets.php" class="card__image">
      <img
        src="img/sets-card.png"
        alt="Роллы"
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    <!-- Цены на товар (с учетом скидки и без)-->
    
    <!-- Ссылка-название товара -->
    <a href="sets.php" class="card__title">
     Сеты
    </a>
    <!-- Кнопка добавить в корзину -->
    <a href=sets.php class="roll-add2"><button class="card__add">Выбрать</button></a>
  </div>
</div>

  <!-- Карточка товара -->
  <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="goryachie.php" class="card__image">
      <img
        src="img/govyadina-card.png"
        alt="Роллы"/>
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    <!-- Цены на товар (с учетом скидки и без)-->
    
    <!-- Ссылка-название товара -->
    <a href="goryachie.php" class="card__title">
     Горячие 
    </a>
    <!-- Кнопка добавить в корзину -->
    <a href="goryachie.php" class="roll-add2"><button class="card__add">Выбрать</button></a>
  </div>
</div>

   
   <!-- Карточка товара -->
     <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="salats.php" class="card__image">
      <img
        src="img/salats-card.png"
        alt="Роллы"/>
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    <!-- Цены на товар (с учетом скидки и без)-->
    
    <!-- Ссылка-название товара -->
    <a href="salats.php" class="card__title">
     Салаты
    </a>
    <!-- Кнопка добавить в корзину -->
    <a href=salats.php class="roll-add2"><button class="card__add">Выбрать</button></a>
  </div>
</div>
   
   <!-- Карточка товара -->
     <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="sope.php" class="card__image">
      <img
        src="img/sope-card.png"
        alt="Роллы"
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    <!-- Цены на товар (с учетом скидки и без)-->
    
    <!-- Ссылка-название товара -->
    <a href="sope.php" class="card__title">
     Супы
    </a>
    <!-- Кнопка добавить в корзину -->
    <a href="sope.php" class="roll-add2"><button class="card__add">Выбрать</button></a>
  </div>
</div>
   
   
   
   <!-- Карточка товара -->
     <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="#" class="card__image">
      <img
        src="img/vok-card.png"
        alt="Роллы"
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    <!-- Цены на товар (с учетом скидки и без)-->
    
    <!-- Ссылка-название товара -->
    <a href="vok.php" class="card__title">
     Вок
    </a>
    <!-- Кнопка добавить в корзину -->
    <a href=salats.php class="roll-add2"><button class="card__add">Выбрать</button></a>
  </div>
</div>
   
   
   
   
   <!-- Карточка товара -->
     <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="tyahan.php" class="card__image">
      <img
        src="img/tyahan-card.png"
        alt="Роллы"
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    <!-- Цены на товар (с учетом скидки и без)-->
    
    <!-- Ссылка-название товара -->
    <a href="tyahan.php" class="card__title">
     Тяхан
    </a>
    <!-- Кнопка добавить в корзину -->
    <a href=salats.php class="roll-add2"><button class="card__add">Выбрать</button></a>
  </div>
</div>
   
   
   
   
   <!-- Карточка товара -->
     <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="poke.php" class="card__image">
      <img
        src="img/poke-card.png"
        alt="Роллы"
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    <!-- Цены на товар (с учетом скидки и без)-->
    
    <!-- Ссылка-название товара -->
    <a href="poke.php" class="card__title">
     Поке
    </a>
    <!-- Кнопка добавить в корзину -->
    <a href=salats.php class="roll-add2"><button class="card__add">Выбрать</button></a>
  </div>
</div>
   
   
   
   
   <!-- Карточка товара -->
     <div class="card">
  <!-- Верхняя часть -->
  <div class="card__top">
    <!-- Изображение-ссылка товара -->
    <a href="drinks.php" class="card__image">
      <img
        src="./img/mors-klukva-card.png"
        alt="Роллы"
      />
    </a>
    <!-- Скидка на товар -->
    <div class="card__label">-10%</div>
  </div>
  <!-- Нижняя часть -->
  <div class="card__bottom">
    <!-- Цены на товар (с учетом скидки и без)-->
    
    <!-- Ссылка-название товара -->
    <a href="drinks.php" class="card__title">
     Напитки
    </a>
    <!-- Кнопка добавить в корзину -->
    <a href=drinks.php class="roll-add2"><button class="card__add">Выбрать</button></a>
  </div>
</div>
   
   
   
   
    </div>





              



        </section>
        

    </main>





    <script src="js/script.js"></script>
    
</body>

</html>